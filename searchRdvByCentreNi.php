...
<?php 
    ...
    $tabmedecin=$bdd->prepare("SELECT * FROM medecin left join centre on (centre.idc = medecin.idc) where nomc = ? ");
    $tabmedecin->execute(array($nomc));
    ...
    $result = $bdd->prepare("SELECT count(*) FROM medecin left join centre on (centre.idc = medecin.idc) where nomc = ?");
    $result->execute(array($nomc));
...
<nav>
    ...
    if($nbm == 0){
        ...
        <h2>il n\'existe aucun medcin dans la centre "'.htmlspecialchars($nomc).'" maintenant<h2>
